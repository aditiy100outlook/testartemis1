...
/**
 * Migration of com.backup42.computer.data.ext.ComputerLicenseDataProvider.findAllActiveUnassignedComputerLicenses() to
 * command structure.
 * 
 * @author bmcguire
 */
public class ComputerLicenseFindAllActiveUnassignedCmd extends DBCmd<List<ComputerLicense>> {
	...
	public List<ComputerLicense> exec(CoreSession session) throws CommandException {
		...
		try {
			...
			return this.db.find(new ComputerLicenseFindAllActiveUnassignedQuery());
	...
	@CoreNamedQuery(name = "findAllActiveUnassignedComputerLicenses", query = "from ComputerLicense as cl where cl.active = true and cl.computerId is null")
	private class ComputerLicenseFindAllActiveUnassignedQuery extends FindQuery<List<ComputerLicense>> {
